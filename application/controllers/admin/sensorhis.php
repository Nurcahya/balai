...
class Sensorhis extends CI_Controller {
	...
	function index()
	{
		...
		$data['current']="sensorhis";
		$sesiid = array(
						'id' 	=> $_GET['id']
						);
		$this->session->set_userdata($sesiid);
		$this->load->view('backend/sensorhis',$data);
	}
	}
	...
	function grid()
	{
		...
		$crud->set_table('history_sensor');
		
		if ($this->session->userdata('id') != '0'){
		$crud->where ('history_sensor.id_pos', $this->session->userdata('id'));
		}
	...
	function tangkapgridsearch()
	{
		...
		redirect('admin/sensorhis/gridsearch/'.$id);
	}
	...
	function gridsearch()
	{
		...
		$crud->where ('history_sensor.id_pos',$this->uri->segment(4));
		}
